...
Use the MoonBit practice skill for MoonBit changes. `mysql`, `postgres_session`, and `moondb_session` depend on the shared
`sql_session` module, never on each other or an application. `ws_session` is independent
...
`SERVICEKIT_TEST_POSTGRES_URL`. All three adapters must pass the shared conformance
...
Run `npm run test:packages` before releases. Keep module LICENSE/NOTICE files in
sync with the Apache-2.0 root license. Registry publication and source-development
submodules are separate workflows; never claim unpublished modules are installable.
