...
A native MoonBit SQL execution/lifetime contract. This module imports only
`moonbitlang/async@0.22.1`; it contains no SQL dialect, database protocol, schema,
or application policies. Both MySQL and PostgreSQL use the same implementation.
...
- `execute(sql, params=[]) -> M` drains any rows and returns command metadata.
  Use `query` for SQL `RETURNING`, or `run` for `Response[R, M]` containing both.
...
not guarantee immediate preemption of native work. Both shipped adapters drain
...
`Driver[C, P, R, M, O]` supplies typed acquire/release/close, run, begin, commit,
...
`acquire` must restore its resources if canceled before returning. `run` and
...
See the [shared real-DB suite](../examples/conformance/src/contract.mbt) and
